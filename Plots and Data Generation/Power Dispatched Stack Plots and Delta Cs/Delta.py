...
#E = np.zeros((len(Gen[0]),len(Gen)-1))
#Conv = np.vstack([Conv,Conv,Conv,Conv,Conv,Conv,Conv,Conv,Conv,Conv])
...
plt.ylabel('Annual Change in Energy Dispatched (TWh)',fontsize=FontSize)
plt.yticks(fontsize=FontSize)
plt.xticks(np.arange(0,2.25,0.25)[::2],fontsize=FontSize)
#plt.suptitle('a)',x=0.05,y=0.99,fontsize=FontSize)
#plt.legend(labels,loc='upper left')
plt.tight_layout()
plt.savefig('2g.png', transparent=True)
plt.savefig('2g.svg', transparent=True)
plt.show()