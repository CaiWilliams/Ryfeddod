...
class Dispatch:
    ...
    def Distribute(self,DC):
        for Asset in DC:
            ...
            OverGen = np.where(MaxGen > DemandRemaining)
            #StorSate = np.where()
            #    return
    ...
    def Storage(self):
        ...
        if np.sum(self.DC2Curtailed) == 0:
            ...
            self.StorageState = np.minimum(StorageState + PosCharge, StorageCapacity)
...
def ScalingDynamFromFile(NG,Solar,SolarNT,EnhancDir):
    NG.Modify('Solar', Scaler=Solar)
